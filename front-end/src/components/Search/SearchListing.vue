<template>
  <div class="container">
    <nav-bar></nav-bar>
    <input type="radio" id="one" value="Jobs" v-model="picked">
    <label for="one"> Search Jobs</label>
    <input type="radio" id="two" value="Contracts" v-model="picked">
    <label for="two">Search Freelance</label>
    <br>
    <div class="container">
      <div class="row">
            <div class="col-sm-6 col-sm-offset-3">
                <div id="imaginary_container">
                    <div class="input-group stylish-input-group">
                        <input type="text" class="form-control" v-model="querydubin" placeholder="Search" >
                        <span class="input-group-addon">
                            <button v-on:click.prevent="searchquery" type="submit">
    ...
    <section class="col-xs-12 col-sm-6 col-md-12">
		<article v-if="this.picked === 'Jobs'" class="search-result row">
			<div v-for="s in searchdata" class="col-xs-12 col-sm-12 col-md-12 excerpet">
        <h3><router-link :to="{ name: 'job', params: { jobid: s.jobid }}">{{s.title}}</router-link></h3>
        <p>{{s.description}}</p>
			</div>
			<span class="clearfix borda"></span>
		</article>
		<article v-else-if="this.picked === 'Contracts'" class="search-result row">
			<div v-for="s in searchdata" class="col-xs-12 col-sm-12 col-md-12 excerpet">
