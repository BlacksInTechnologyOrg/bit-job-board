...
class MessageHandler:
    ...
    def getAllMessages(self,username, pagenumber):
        ...
        try:
            messages = Message.objects(Q(author__exact=username) | Q(recipient__exact=username))\
                .order_by('conversationId','modified_at').skip(offset).limit(items_per_page).to_json()
            return messages
        except:
